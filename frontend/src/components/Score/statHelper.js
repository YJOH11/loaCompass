...
const upperStats = {
  투구: [0,234,470,708,948,1189,1431,1675,1921,2168,2417,2667,2919,3173,3429,3687,3945,4205,4467,4731,4999],
  견갑: [0,249,500,753,1008,1265,1523,1783,2045,2309,2573,2839,3107,3377,3649,3924,4199,4476,4755,5036,5320],
  상의: [0,187,376,566,758,951,1145,1340,1537,1735,1934,2134,2336,2539,2744,2950,3157,3365,3575,3786,3999],
  하의: [0,202,406,611,818,1027,1236,1447,1659,1873,2089,2305,2523,2742,2963,3186,3409,3634,3861,4090,4320],
  장갑: [0,281,564,849,1136,1426,1716,2009,2304,2601,2900,3200,3503,3808,4115,4424,4734,5046,5361,5678,5998],
  무기: [
...
export function applyAccessoryEffects({ basicEffect, refinementEffects }, myStat) {
  // 부동소수점 오류 방지용 정밀 계산 함수
   const addWithPrecision = (target, value, label = "") => {
      ...
      console.log(`[${label}] ${target} + ${value} = ${result}`);
      return result;
    };
...
export function applyAbilityStones(abilityStoneEngravings, myStat) {
  abilityStoneEngravings.forEach((str) => {
    ...
    myStat[statInfo.statKey] += value;  // 그대로 더함
